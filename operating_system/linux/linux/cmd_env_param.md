...
envp=PWD=/code/demo/cmd_env_param
envp=SHLVL=1
envp=HOME=/root
envp=_=./cmd_env_param
envp=OLDPWD=/code/demo
```

argc的值就是命令行参数，包括执行命令和跟在后面的三个参数。

envp的值就是环境参数，环境参数也可以通过`env`命令输出。
